...
## 4. Value Objects для исторических данных

`ParameterSnapshot` и `ResultSnapshot` хранят состояние методики на момент создания исследования, обеспечивая:

- независимость от будущих изменений в `StudyTemplate`;
- быстрый доступ к историческим данным без JOIN-запросов;
- возможность аудита "какая методика применялась в прошлом".
