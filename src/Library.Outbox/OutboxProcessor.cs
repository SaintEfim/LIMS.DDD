...
public sealed class OutboxProcessor(DbContext context, IMessageBus messageBus, ILogger<OutboxProcessor> logger)
{
    private const int BatchSize = 10;

    private static readonly MethodInfo SendAsyncMethod = typeof(IMessageBus).GetMethod(nameof(IMessageBus.SendAsync))!;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var message in outboxMessages)
        {
            ...
            try
            {
                ...
                var genericSendMethod = SendAsyncMethod.MakeGenericMethod(messageType);
