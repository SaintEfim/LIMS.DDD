...
public sealed class StudyCompletedState : IState<Study>
{
    ...
    public Result<Exception> CanTransitionTo(IState<Study> newState, Study study)
    {
        ...
        {
            StudyCompletedState or StudyApprovedState or StudyInProgressState => Result<Exception>.Success(),
            _ => Result<Exception>.Failure(new InvalidOperationException("Invalid transition from Completed"))
        };
    }
}
